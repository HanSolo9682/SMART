...
from src.models.lamra_ret import MMEBEmbeddingModel
...
def _format_batch_inputs(model: MMEBEmbeddingModel, batch_inputs):
    return [
        model.encoder.format_model_input(
            text=ele.get("text"),
            image=ele.get("image"),
            video=ele.get("video"),
...
def encode_anchor_and_tokens(
    ...
    anchors, hidden_state, attention_mask = encoder.forward(processed_inputs)
    if model.normalize:
        anchors = F.normalize(anchors, p=2, dim=-1)

    input_ids = processed_inputs["input_ids"]
    # if encode_side == "qry":
    #     keep_mask = _late_keep_token_mask_text_only(input_ids, attention_mask, encoder.processor.tokenizer)
    # else:
    keep_mask = _late_keep_token_mask(input_ids, attention_mask, encoder.processor.tokenizer)

    with torch.autocast(device_type="cuda", enabled=False):
        normalized_hidden = F.normalize(hidden_state.float(), p=2, dim=-1)
    token_states = []
...
def encode_representations(
    model, # Assumed MMEBEmbeddingModel
...
def main():
    ...
    model.encoder.model = model.encoder.model.to(eval_args.device, dtype=torch.bfloat16)
