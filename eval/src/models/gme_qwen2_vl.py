...
class GmeQwen2VL:
    ...
    def forward(
        ...
        **kwargs
        ...
        # Returning both the pooled embedding and the full hidden states
        return embeddings.contiguous(), outputs.last_hidden_state
    ...
    def embed(self, texts: list[str], images: list[Image.Image], is_query=True, instruction=None, **kwargs):
        ...
        with torch.no_grad():
            embeddings, last_hidden_state = self.forward(**inputs)
            
        # Return all components needed for late interaction eval
        return embeddings, last_hidden_state, inputs['input_ids'], inputs['attention_mask']

    def compute_similarity(self, q, p):
        # Enables direct compatibility with the eval script's batched ranking calculations
        return torch.matmul(q, p.transpose(-1, -2))
    ...
    def get_fused_embeddings(self, texts: list[str] = None, images: list[Image.Image] | DataLoader = None, **kwargs):
        ...
        for n, img_batch in zip(range(0, n_batch * batch_size, batch_size), image_loader):
            ...
            # Unpack the tuple here so the base encode methods don't break
            embeddings, _, _, _ = self.embed(texts=text_batch, images=img_batch, **kwargs)
...
def smart_resize(
    height: int, width: int, factor: int = IMAGE_FACTOR, min_pixels: int = MIN_PIXELS, max_pixels: int = MAX_PIXELS) -> tuple[int, int]:
